...
func (src *Src) Process(dataIn []float32, ratio float64, endOfInput bool) ([]float32, error) {
	...
	return (*[1 << 30]float32)(unsafe.Pointer(&src.outputBuffer[0]))[: src.srcData.output_frames_gen*src.channels : src.srcData.output_frames_gen*src.channels], nil
}
...
func GetName(converter int) (string, error) {
	cConverterName := C.src_get_name(C.int(converter))
...
func GetDescription(converter int) (string, error) {
	cConverterDescription := C.src_get_description(C.int(converter))
