...
func NewWindow(title string, width int, height int, hasMenubar bool) *Window {
	...
	C.pkguiWindowOnContentSizeChanged(w.w)
...
// OnContentSizeChanged registers f to be run
// when the Window's content size is changed.
// Only one function can be registered at a time.
func (w *Window) OnContentSizeChanged(f func(*Window)) {
	w.onSizeChanged = f
}

//export pkguiDoWindowOnContentSizeChanged
func pkguiDoWindowOnContentSizeChanged(ww *C.uiWindow, data unsafe.Pointer) {
	w := ControlFromLibui(uintptr(unsafe.Pointer(ww))).(*Window)
	if w.onSizeChanged != nil {
		w.onSizeChanged(w)
	}
}

// Fullscreen returns whether the Window is full screen.
func (w *Window) Fullscreen() bool {
	return tobool(C.uiWindowFullscreen(w.w))
}

// SetFullscreen sets the Window to be full screen or not.
func (w *Window) SetFullscreen(fullscreen bool) {
	C.uiWindowSetFullscreen(w.w, frombool(fullscreen))
}
